...
class Order(Base):
    ...
    __table_args__ = (
        Index("uq_order_org_number", "organization_id", "order_number", unique=True),
        # Hot path for every analytics query: org + status + placed_at range.
        Index("ix_orders_org_status_placed", "organization_id", "status", "placed_at"),
    )
